(function ($) {
  $.fn.jTimeline = function (options) {
    var settings = $.extend(
      {
        ...
        leftArrow: '<i class="fas fa-arrow-left"></i>', // Font Awesome left arrow
        rightArrow: '<i class="fas fa-arrow-right"></i>', // Font Awesome right arrow
      },
      options
    );
    ...
    return this.each(function () {
      ...
      $timeline
        .prepend(
          '<div class="jtimeline-scroll-left"><div class="jtimeline-arrow">' +
            settings.leftArrow +
            "</div></div>"
        )
        .append(
          '<div class="jtimeline-scroll-right"><div class="jtimeline-arrow">' +
            settings.rightArrow +
            "</div></div>"
        );
      ...
      $events.each(function () {
        ...
        $event
          ...
          .append(
            '<div class="jtimeline-event-label">' + eventLabel + "</div>"
          );
      });
      ...
      if (typeof start === "number" && typeof end === "number") {
        ...
        $events.each(function () {
          var $event = $(this),
            ...
            position =
              timeOffset / settings.resolution -
              eventWidth / 2 +
              cumulativeSpacing;

          if (
            typeof prevEventPosition === "number" &&
            typeof prevEventTimestamp === "number"
          ) {
          ...
          if (isActive) {
            if (position > timelineWidth) {
              initialX =
                timelineWidth - position - timelineWidth / 2 - eventWidth / 2;
            }
          }
        });
        ...
        $scrollLeft.on("click", function (e) {
          ...
          var transform = $eventsList.css("transform"),
            currentX =
              transform === "none"
                ? 0
                : parseInt(transform.split(/[()]/)[1].split(", ")[4]),
